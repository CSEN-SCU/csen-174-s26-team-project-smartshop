...
*Consequences:* No single URL surfaces all four features. A user wanting price chat AND product alternatives must run two local servers. Three teammates' work is invisible at the deployed URL — accepted for the prototype sprint but blocks any full-product user test.
